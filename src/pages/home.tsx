...
export const Home: React.FC = () => {
  ...
  const handleAnswerExam = (letter: string) => {
  ...
  const handleNextExam = () => {
    if (selectedAnswer) {
      ...
      setAnswersHistory(prev => [
        ...prev,
        { question: currentQuestion, chosen: selectedAnswer },
      ]);
  ...
  const handleAnswerPractice = (letter: string) => {
    if (!selectedAnswer) {
      setSelectedAnswer(letter);
      setUsedQuestions(prev => [...new Set([...prev, questions[currentIndex].question_number])]);
    }
  };
  ...
  const handleReviewMissed = () => {
    ...
    const missed = answersHistory.filter(
      entry => entry.chosen !== entry.question.correct_answer
    );
  ...
  if (!mode) {
    return (
      <div className="min-h-screen w-screen flex flex-col justify-stretch py-5 items-center bg-[#181d1c] px-2">
        <h1 className="text-2xl md:text-3xl font-bold text-center text-white">
          GCP Associate Cloud Engineer Preparation
        </h1>
  ...
  if (mode === 'exam' && currentIndex >= questions.length) {
    ...
    return (
      <div className="min-h-screen w-screen flex flex-col items-center justify-center bg-[#181d1c] px-2">
        ...
        <div className="w-full max-w-4xl">
          ...
          <div className="flex flex-col items-center gap-4 mb-6">
            <button
              ...
              disabled={answersHistory.filter(entry => entry.chosen !== entry.question.correct_answer).length === 0}
            >
                  ...
                  <li key={entry.question.question_number} className="border-b border-[#374140] pb-6 last:border-b-0">
                    ...
                    <div className="flex flex-col gap-2">
                      {entry.question.answers.map((ans, i) => {
                        ...
                        const isCorrect = letter === entry.question.correct_answer;
    ...
    <div className="min-h-screen w-screen flex flex-col items-center justify-center bg-[#181d1c] px-2">
      ...
      <ul className="space-y-3 w-full max-w-3xl">
        {currentQuestion.answers.map((answer, idx) => {
          ...
          // Selected state (radio effect)
          if (isSelected && !(mode === "practice" && selectedAnswer)) {
            ring =
              "ring-2 ring-[#4db6ac] ";
            bg = "bg-[#374140]";
          }
          ...
          return (
            <li key={letter} className='min-w-full'>
              <button
                disabled={
                  (mode === "practice" && selectedAnswer !== null) 
                }
                onClick={() =>
                  mode === "exam"
                    ? handleAnswerExam(letter)
        ...
        {mode === 'exam' && (
          <button
            ...
            className={`ml-auto px-6 py-2 rounded bg-green-600 text-white font-semibold transition-all duration-150 ${
              !isLocked ? "opacity-50 cursor-not-allowed" : "hover:bg-green-700"
            }`}
          >
            Next
          </button>
